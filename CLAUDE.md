...
- **centuria/mappa-3d-prova.html** — Sandbox page for the experimental 3D world map (not linked from the site yet). See "Centuria 3D World Map (experimental)" below.
...
- **External Dependencies**: The site relies on Google Fonts, Leaflet.js and three.js (via an import map) from CDNs. Ensure `<link>` and `<script>` tags remain in HTML `<head>` sections.
...
## Centuria 3D World Map (experimental)

Work in progress, meant to eventually replace the Leaflet map on `centuria.html`; for now it lives only on the sandbox page `centuria/mappa-3d-prova.html`. `centuria/js/world-map.js` renders the map as a tilted three.js diorama modelled on the painting `centuria/resources/Centuria.png` (bronze landmasses, slate sea with gilded rhumb lines, instanced mountains/forests, brass gears, compass rose, clouds over Terre Ignote). Props (gears, compass, Kassendyr citadel, capital towers) live in `centuria/js/world-map-props.js`; styles in `centuria/css/world-map.css`. Nation labels are HTML buttons projected over the canvas and open a popup.

- **Terrain comes from a baked mask**, `centuria/resources/map/centuria-terrain.png` (R = land, G = mountains, B = forest), produced by `node tools/centuria-map/bake-terrain.mjs` (dependency-free, own PNG codec in `png.mjs`). Land is traced from the painting's warm hue; ornament exclusions, lakes, mountain and forest polygons are hand-authored in the painting's pixel space inside the bake script. Edit the script and re-run it — don't hand-edit the PNG.
- **All positions in `world-map.js` (`NATIONS`, `OCEANS`, gears) are painting pixels** (1672 × 941, origin top-left) so they can be read straight off `Centuria.png`.
- Placement of mountains/trees uses a seeded PRNG: same map on every load.
...
### Deep-space phenomena (nebulae, clusters, pulsar, quasar)

`unknown-frontier/js/space-phenomena.js` (`createSpacePhenomena({ scene, camera, galaxies })`, `update(delta, elapsed)` per frame) adds decorative, non-clickable scenery: large nebula complexes as a far backdrop (r ≈ 400–550), plus planetary nebulae, a supernova remnant, globular/open clusters, a pulsar and a quasar in the space between galaxies. Textures are seeded procedural canvas bakes in `unknown-frontier/js/nebula-texture.js` (same look on every reload).

- **Galaxies must stay readable.** Mid-range objects sit off the galactic band (|y| ≥ 60, away from the y-axis column), and every phenomenon is dimmed per frame when it overlaps a galaxy on screen (angular test using live galaxy positions/radii from `GALAXY_LABELS`). Big complexes only test their core (`overlapRadius`), otherwise they'd stay permanently dimmed.
- **Nebula textures are opaque color-on-black, not alpha.** Low-alpha canvas pixels get premultiply-quantized into muddy grey once uploaded. They're drawn with `CustomBlending` (additive color, framebuffer alpha untouched) — plain `AdditiveBlending` writes alpha = 1 over the whole quad and punches dark squares into the transparent renderer canvas over the page's CSS background.
- **`python -m http.server` lets the browser cache ES modules** — edited `.js` files may not reload. Serve with a `Cache-Control: no-store` header when iterating.

### Planet pages (generated) & alien civilizations

`unknown-frontier/systems/planets/*.html` (all planets except the hand-written Sol pages) are **generated** by `node tools/planet-pages/generate.mjs` from `js/solar-system-data.js`. Re-running overwrites them — edit the data, not the HTML.

- **Uninhabited planets:** tabs Panoramica / Visione Dettagliata / Biosfera / Pericoli / Rilevamenti, assembled from per-classification pools in `tools/planet-pages/type-lore.mjs` with a per-slug seeded RNG (stable output). Fauna/flora names are a generated alien root + pool label.
- **Inhabited planets:** Terra-style tabs (Panoramica, Visione Dettagliata, Specie, Civiltà, Relazioni, Luoghi & Personaggi, Mercato) from `tools/planet-pages/races/{race}.mjs` — race-wide species/civilization/relations/market plus per-planet description, places, characters, legend, globe POIs. Registry in `races/index.mjs`; an inhabited planet with no race entry keeps its old page.
- **Ten civilizations:** Terrestri (Ferrandis colonies), Velmyr, Ythar, Coralith (the last three match the starship lab), Vaelun, Quorai, Kheprani, Nexari, Thissari, Ashkaari. Every race's `relations` covers the other nine and the files cross-reference shared events (e.g. the Ythar spores at Revaix Cradle, the Coralith–Kheprani War of the Glass Sands) — keep both sides consistent when editing.
- **Race-themed markets:** Terrestri reuse `market-industrial.css` (blast door). Alien markets use `.market-shell.market-{race}` markup: load `market-industrial.css` (unscoped carousel layout driven by `--mk-*` tokens), then `market-shell.css` (shared structure), then `market-{race}.css` (tokens, skin, `.market-reveal` open animation). `js/market-carousel.js` handles both.
- **Globe for invented worlds:** `js/planet-globe.js` paints the same procedural texture as the system view when `data-texture` is absent (`data-slug`/`data-type`/`data-color`/`data-size`), with optional `data-ring-color`.
